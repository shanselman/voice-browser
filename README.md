...
8. For better latency/reliability, run local STT: `.\start.ps1 -SttBackend faster-whisper`.
9. If GPU runtime libraries are missing, local STT now auto-falls back to CPU int8.
10. If local STT has repeated backend errors, Voice Browser auto-switches to Google STT for that session.
11. For long, slower thoughts, increase capture windows: `.\start.ps1 -PhraseLimit 40 -PauseThreshold 1.6`.
12. To diagnose local STT failures, run with `-SttDebug`.
